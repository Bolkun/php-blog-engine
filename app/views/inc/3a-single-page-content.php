<div class="container">
    <div style="margin-top: -6px; z-index: -1;">
        <?php if (isAdminLoggedIn() === true) { ?>
            <form id="blog_form" style="z-index: 0;" class="form-inline" action="" method="post" enctype="multipart/form-data">
                <div class="form-row" style="width: 100%;">
                    ...
                    <div class="form-group col-md-6">
                        ...
                        <input id="blog_category" type="text" name="blog_category" class="form-control <?php echo (!empty($data['blog_category_err'])) ? 'is-invalid' : ''; ?>" value="<?php echo $data['blog_category']; ?>" placeholder="Category">
                        <span class="invalid-feedback"><?php echo $data['blog_category_err']; ?></span>
                    </div>
                    <div class="form-group col-md-6">
                        <label for="blog_title">Title</label>
                        <input id="blog_title" type="text" name="blog_title" class="form-control <?php echo (!empty($data['blog_title_err'])) ? 'is-invalid' : ''; ?>" value="<?php echo $data['blog_title']; ?>" placeholder="Title">
                    ...
                    <div class="form-group col-md-6" style="padding-bottom: 78px;">
                        ...
                        <div id="blog_preview_image_server_div" class="custom-file">
                            <input id="blog_preview_image_server" type="text" name="blog_preview_image_server" onclick='ajax_loadPreviewImageList(<?php echo jsonEncodeURLRoot(); ?>)' class="form-control <?php echo (!empty($data['blog_preview_image_err'])) ? 'is-invalid' : ''; ?>" value="<?php echo $data['blog_preview_image']; ?>" data-toggle="modal" data-target="#blog_preview_images_list" readonly>
                                                ...
                                                <div id="blog_preview_images_list_load_content">
                                                    ...
                                                    <div class="row">
                                                        <?php for ($p = 0; $p < count($data['preview_image_list']); $p++) { ?>
                                                            <div class="col-sm-2">
                                                                <img style="border: 1px solid rgba(0, 0, 0, 0.5);" onclick='selectedPreviewImage(<?php echo jsonSelectedPreviewImage($data['preview_image_list'][$p]); ?>)' class="blog_preview_img" src="<?php echo PUBLIC_CORE_IMG_PREVIEWURL . '/' . $data['preview_image_list'][$p]; ?>">
                                                                <?php if (DEFAULT_PREVIEW_IMAGE !== $data['preview_image_list'][$p]) { ?>
                                                                    <div class="img-trash" onclick='ajax_deletePreviewImage(<?php echo jsonSelectedPreviewImage($data['preview_image_list'][$p]); ?>)'>
                                                                        <span>
                        ...
                        <div style="display: none;" id="blog_preview_image_local_div" class="custom-file">
                            <input id="blog_preview_image" onchange="selectedUploadPreviewImage(this)" type="file" name="blog_preview_image_local" accept="image/.jpg,.png,.jpeg,.gif,.svg" class="custom-file-input <?php echo (!empty($data['blog_preview_image_err'])) ? 'is-invalid' : ''; ?>">
